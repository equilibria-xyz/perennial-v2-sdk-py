...
def getType(lst, t):
    ...
    for item in lst:
        ...
        if item_type == "tuple":
            ...
            component_type = f"({getType(item['components'], True)})"
...
def decodeCalldata(data, func, abi):
    funcFragment = next(filter(lambda v: v["name"] == func, abi), None)
    outputs = funcFragment["outputs"]

    ## Convert outputs to a types string
    types = getType(outputs, False)
