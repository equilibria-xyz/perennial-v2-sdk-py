...
def approve_usdc_to_dsu(collateral_amount: float) -> HexBytes:
    try:
...
def commit_price_to_multi_invoker(market_address: str) -> HexBytes:
    try:
...
def close_position_in_market(market_address: str) -> HexBytes:
    try:
...
def withdraw_collateral(market_address: str, snapshot: dict = None) -> HexBytes:
    try:
        if not snapshot:
...
def deposit_collateral(market_address: str, collateral_amount: float) -> HexBytes:
    try:
        amount_usdc = int(collateral_amount * 10 ** 6)
...
def place_limit_order(market_address: str, side: int, price: float, delta: float) -> HexBytes:
    try:
        global comparison
...
def cancel_order(market_address: str, nonce: int) -> HexBytes:
    try:
